...
int main(int argc,char * argv[])
{
    ...
    rayTracer = new RayTracer(sceneParser, InputParser::max_bounces, InputParser::cutoff_weight, InputParser::shadows, InputParser::shade_back);

    if(InputParser::gui)
    {
        GLCanvas canvas;
        glutInit(&argc,argv);
		canvas.initialize(sceneParser, render, trace);
        delete sceneParser;
    }
    else{
        render();
